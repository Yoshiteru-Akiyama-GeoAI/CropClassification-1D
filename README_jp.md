...
> 本リポジトリは公開データセットを用いた分類実験の参考結果を掲載している。データセットはtrain/validation/testへランダム分割して使用しており、処理プログラムは公開しているものの、実際に使用した分割データ自体は公開していない。そのため、実行環境や分割条件の違いにより、同一手法であっても掲載結果と完全には一致しない場合がある。掲載結果は、著者環境における一例として参照されたい。
...
本ベンチマークでは、Sentinel-2による作物分類を対象として、時系列モデリング手法を体系的に比較した。

実験結果から、以下のことが明らかとなった。

* 最適な時系列表現方法は、モデルアーキテクチャに依存する。
* 明示的な時系列情報はリカレントモデルにおいて有効である一方、今回評価したTransformerでは必ずしも必要ではない。
* 可変長の衛星時系列データに対しては、集約方法（Aggregation strategy）が重要な役割を果たす。
* 評価したアーキテクチャの中では、Self-Attentionが最も高い総合性能を示した。

これらの知見は、衛星データを用いた作物分類における時系列モデリング手法の選択に関する実践的な指針を提供するものである。
